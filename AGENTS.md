...
* Forms that stay on the page after a successful action (e.g., SettingsForm) must use CONTROLLED inputs (`value` + `onChange`), never `defaultValue`. React 19 auto-resets uncontrolled fields to their `defaultValue` when a form action completes, which makes saved values appear to revert until a manual refresh. Forms that redirect after save are unaffected.
...
* Follow-up jobs created by `scheduleFollowUpJob` derive `job_inputs` from the property's default service booleans (the customer's agreed ongoing service plan), with add-ons reset to none. The parent job's `job_inputs` is only the fallback when the property has no boolean selections. Do not copy one-off parent-job scope (e.g., a leaf cleanup add-on) onto recurring follow-ups. Changing a recurring job's scope requires a new estimate accepted by the customer (the `sets_property_defaults` flow), which updates the property booleans at approval — follow-ups then inherit the new scope automatically. (Decided 2026-08-03; supersedes the earlier "copy parent job_inputs" rule.)
...
* Google Voice SMS mode (field-verified 2026-08-12): the Android share sheet (`navigator.share` with the message text) is the ONLY reliable web → Google Voice app handoff, and GV pre-fills the shared text. Do NOT retry: VIEW `intent://` to voice.google.com URLs (GV registers no filters for its own web URLs), MAIN/LAUNCHER `intent://` (Chromium sanitizes to BROWSABLE-only; GV has none), or web GV on mobile (drops the `?itemId` thread param and renders desktop-only). Every GV-mode button must carry a body — bare Text buttons pass a starter greeting. Web thread URLs are a desktop-only fallback.
